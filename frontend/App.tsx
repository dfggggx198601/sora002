...
const App = () => {
  ...
  const runGenerationInBackground = async (taskId: string, config: GenerationConfig) => {
    try {
      ...
      const isTimeout = err.message && (err.message.includes('504') || err.message.includes('524') || err.message.includes('timeout'));
      ...
      if (err.name === 'AbortError' || err.message?.includes('aborted') || isTimeout || isSyntaxError) {
        console.warn('[Auto-Recovery] Request timed out or invalid response (likely HTML). Switching to background polling mode. Error:', err.message);
        return;
      }
